...
describe('GET /agendamentos', () => {
  ...
  test('deve listar agendamentos para admin', async () => {
    mockQuery([]);
    mockQuery([{ id: 1, cliente: 'João', veiculo: 'Corolla', servico: 'Lavagem', data: new Date(), status: 'pendente' }]);
...
describe('PUT /agendamentos/:id', () => {
  ...
  test('deve atualizar status com sucesso e notificar cliente', async () => {
    mockQuery([{ id: 1, status: 'aprovado', data: new Date() }]);
    // mock para buscar dados do cliente para o email
