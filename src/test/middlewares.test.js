...
describe('authMiddleware', () => {
  ...
  test('deve retornar 401 sem header Authorization', async () => {
...
describe('GET /cliente/minha-conta', () => {
  ...
  test('deve retornar dados da conta', async () => {
    ...
    mockQuery([{
      ...
      telefone: '11999999999', data_nascimento: null,
      cep: null, logradouro: null, numero: null,
      complemento: null, bairro: null, cidade: null,
      estado: null, perfil_completo: false, criado_em: new Date()
    }]);
...
describe('DELETE /cliente/minha-conta (LGPD)', () => {
  ...
  test('deve anonimizar e deletar usuário', async () => {
    const token = jwt.sign({ id: 2, tipo: 'cliente', email: 'cli@test.com', cliente_id: 5 }, SECRET, { expiresIn: '1d' });
...
describe('GET /cliente/meus-veiculos', () => {

  test('deve retornar veículos do cliente', async () => {
    const token = jwt.sign({ id: 2, tipo: 'cliente', email: 'cli@test.com', cliente_id: 5 }, SECRET, { expiresIn: '1d' });

    mockQuery([{ id: 5, email: 'cli@test.com' }]);
    mockQuery([{ id: 1, modelo: 'Corolla', placa: 'ABC1234', cor: 'Branco', ano: 2020, marca: 'Toyota' }]);
    ...
    expect(res.status).toBe(200);
    expect(res.body[0]).toHaveProperty('placa', 'ABC1234');
  });
});
