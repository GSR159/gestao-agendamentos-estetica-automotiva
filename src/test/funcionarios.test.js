...
describe('GET /funcionarios/ativos', () => {
  ...
  test('deve listar funcionários ativos', async () => {
    mockQuery([{ id: 1, nome: 'João Silva' }, { id: 2, nome: 'Maria Santos' }]);
...
describe('GET /funcionarios', () => {
  ...
  test('deve listar todos para admin', async () => {
    mockQuery([
      { id: 1, nome: 'João', ativo: true,  criado_em: new Date(), agendamentos_hoje: 2 },
      { id: 2, nome: 'Maria', ativo: false, criado_em: new Date(), agendamentos_hoje: 0 },
    ]);
...
describe('POST /funcionarios', () => {
  ...
  test('deve retornar 400 se nome estiver vazio', async () => {
    const res = await request(app)
      .post('/funcionarios')
      .set('Authorization', `Bearer ${tokenAdmin}`)
      .send({ nome: '' });
...
describe('PUT /funcionarios/:id', () => {
  ...
  test('deve retornar 400 se nenhum campo enviado', async () => {
    const res = await request(app)
      .put('/funcionarios/1')
      .set('Authorization', `Bearer ${tokenAdmin}`)
      .send({});
  ...
  test('deve atualizar com sucesso', async () => {
    ...
    const res = await request(app)
      .put('/funcionarios/1')
      .set('Authorization', `Bearer ${tokenAdmin}`)
      .send({ nome: 'João Atualizado', ativo: false });
...
describe('DELETE /funcionarios/:id', () => {
  ...
  test('deve retornar 400 se tiver agendamentos futuros', async () => {
    mockQuery([{ count: '2' }]);
    const res = await request(app)
      .delete('/funcionarios/1')
      .set('Authorization', `Bearer ${tokenAdmin}`);
    expect(res.status).toBe(400);
    expect(res.body.erro).toMatch(/agendamentos futuros/i);
  });
