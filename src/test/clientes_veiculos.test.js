...
describe('GET /clientes', () => {
  ...
  test('deve retornar lista de clientes', async () => {
    mockQuery([
      { id: 1, nome: 'João Silva', email: 'joao@test.com', telefone: '11999' },
      { id: 2, nome: 'Maria Lima', email: 'maria@test.com', telefone: '11888' },
    ]);
...
describe('POST /clientes', () => {
  ...
  test('deve criar cliente com sucesso', async () => {
    mockQuery([{ id: 5, nome: 'Novo Cliente', email: 'novo@test.com', telefone: '11777' }]);

    const res = await request(app)
      .post('/clientes')
      .set('Authorization', `Bearer ${tokenAdmin}`)
      .send({ nome: 'Novo Cliente', email: 'novo@test.com', telefone: '11777' });
...
describe('DELETE /clientes/:id (LGPD)', () => {
  ...
  test('deve anonimizar cliente (LGPD)', async () => {
...
describe('GET /veiculos', () => {
  ...
  test('deve listar veículos para admin', async () => {
    mockQuery([{ id: 1, cliente_id: 1, marca: 'Toyota', modelo: 'Corolla', placa: 'ABC1234', cor: 'Branco', ano: 2020, cliente: 'João' }]);
...
describe('POST /veiculos', () => {
  ...
  test('deve criar veículo com sucesso', async () => {
    mockQuery([{ id: 10, cliente_id: 1, modelo: 'Corolla', placa: 'ABC1234', marca: 'Toyota', cor: 'Branco', ano: 2020 }]);
...
describe('DELETE /veiculos/:id', () => {
  ...
  test('deve deletar veículo como admin', async () => {
    mockQuery([{ id: 1 }]);
    const res = await request(app)
      .delete('/veiculos/1')
      .set('Authorization', `Bearer ${tokenAdmin}`);
